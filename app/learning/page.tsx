        ...
        {projectsData.map((item, index) => (
          <li key={item.title} className="py-6 first:pt-0">
            <div className="flex items-start gap-4">
              {/* Index number */}
              <span className="mt-1 w-6 shrink-0 font-mono text-xs text-gray-400 dark:text-gray-600">
                {String(index + 1).padStart(2, '0')}
              </span>

              {/* Text content */}
              <div className="min-w-0 flex-1">
                <h2 className="mb-1 text-base font-semibold leading-snug tracking-tight text-gray-900 dark:text-gray-100">
                  {item.href ? (
                    <Link
                      href={item.href}
