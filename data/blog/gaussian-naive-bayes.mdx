...
![likelihood_diff_mean](/static/images/resource/gaussian-naive-bayes/likelihood_diff_mean.png)
...
![likelihood_diff_var](/static/images/resource/gaussian-naive-bayes/likelihood_diff_var.png)
...
$$
{p(x)} = {\sum_{c=1}^{3}p(x|c)p(c)} \\ = p(x|c_1)p(c_1) \times p(x|c_2)p(c_2) \times p(x|c_3)p(c_3)
$$
...
![predicted](/static/images/resource/gaussian-naive-bayes/predicted.png)

## Reference

1. Duda, R. O., Hart, P. E., & Stork, D. G. (2001). Chapter 2: Bayesian decision theory. In *Pattern Classification* (pp. Page Numbers). Wiley.
